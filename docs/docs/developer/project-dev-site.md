...
It is not about deploying your own AMS instance — see [Deployment](deployment.md) for that, or the [Provisioning runbook](../hosting/provisioning-runbook.md) if you're the provider standing up a client instance on the project's DigitalOcean stack.
...
It's a useful syntax reference for the DigitalOcean App Platform spec format, but it describes only this one site, not the environment-split (UAT/production) structure a client deployment uses — see the [Provisioning runbook](../hosting/provisioning-runbook.md#2-server-setup-digitalocean-app-platform) for that.
