# Association Management Software (AMS)

## What is Association Management Software?

AMS is a comprehensive, open-source platform designed specifically for membership associations, professional bodies, clubs, and community organisations.
Built with Django and Wagtail CMS, AMS provides everything you need to manage your association in one integrated system.

Developed by the Digital Technologies Teachers Aotearoa (DTTA) to meet their own operational needs, AMS brings together membership management, integrated billing, content publishing, and community engagement in a single, cohesive platform.

<!-- TODO screenshot: Member dashboard overview showing membership status, profile completion, and quick actions -->

Want to learn more?

- **[Read more about the features of AMS](features.md)**

## Find your section

- **Considering AMS, or want the full feature list?** → [Features](features.md)
- **Onboarding a new client — decisions, costs, accounts?** → [Getting started](getting-started/index.md)
- **Setting up or running your association's website?** → [Your website](website/index.md)
- **Provisioning or hosting an AMS instance?** → [Hosting AMS](hosting/index.md)
- **Developing or contributing to AMS itself?** → [Developing AMS](developer/index.md)

<!-- markdownlint-disable -->
<style>
.md-sidebar--primary { display: none;}
</style>
