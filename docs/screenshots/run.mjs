...
// that goes stale" instinct as getEnglishHomePageId's runtime lookup
// (bullet 13), applied to a date instead of a database ID. Computed fresh
...
// elsewhere in this suite -- see fillBodyText's own comment) exposes an
// editor API rather than a plain contenteditable, and setContent() is its
// own documented way to set content programmatically. Confirmed live: this
// syncs the underlying hidden textarea a form submit actually reads from
// immediately, with none of fillBodyText's debounce race to guard against.
...
// the same "only one exists" simplification openFirstIndividualMembership
// relies on above.
