...
- **NPM Dependencies:** When adding or updating npm packages in `package.json`, restart the node container with `docker-compose restart node`. The container's entrypoint script automatically detects changes to `package.json` and runs `npm install` if needed. No manual image rebuild or volume cleanup is required.
